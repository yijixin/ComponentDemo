...
import com.uidt.module_custom_view.adapter.BaseDelegateAdapter;
import com.uidt.module_custom_view.adapter.BaseViewHolder;
import com.uidt.module_custom_view.adapter.GlideImageLoader;
...
public class VlayoutActivity extends BaseActivity {
    ...
    public static void startAction(Activity activity) {
        Intent intent = new Intent(activity, VlayoutActivity.class);
    ...
    protected View getTopView() {
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.img_title_bar);
        int pixelColor = bitmap.getPixel(0, 0);
        ivTitleBar.setBackgroundColor(pixelColor);
    ...
    public void initViews(Bundle savedInstanceState) {
        ...
        ivTitleBar = findViewById(R.id.iv_title_bar);
        RecyclerView recyclerView = findViewById(R.id.recycler_custom_vlayout);
        ...
        BaseDelegateAdapter baseMenuAdapter = new BaseDelegateAdapter(this,gridLayoutHelper,R.layout.vlayout_menu,10,1){
            @Override
        ...
        //新闻区域
        BaseDelegateAdapter baseNewAdapter = new BaseDelegateAdapter(this,new LinearLayoutHelper(),R.layout.vlayout_news,1,2){
            @Override
            public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
                super.onBindViewHolder(holder, position);
        ...
        delegateAdapter.addAdapter(baseNewAdapter);

        //热门商品
        for (int i = 0; i < ITEM_URL.length; i++) {
            final int finalI = i;
            BaseDelegateAdapter titleAdapter = new BaseDelegateAdapter(this,new LinearLayoutHelper(),R.layout.vlayout_title,1,3){
                @Override
                public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
                    super.onBindViewHolder(holder, position);
            ...
            BaseDelegateAdapter gridAdapter = new BaseDelegateAdapter(this, new GridLayoutHelper(2),
                ...
                public void onBindViewHolder(@NonNull BaseViewHolder baseViewHolder, int position) {
                    baseViewHolder.setImageResource(R.id.iv, GRID_URL[position]);
...
//                goodIv.setImageResource(R.mipmap.flashsale1);
                Glide.with(VlayoutActivity.this).load(goods[position % 5]).placeholder(R.mipmap.flashsale1).into(goodIv);
                super.onBindViewHolder(holder, position);
            }
        };
